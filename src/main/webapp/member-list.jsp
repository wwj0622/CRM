...
<div class="page-container" id="app">    
	...
	<div class="text-c"> 日期范围：
		...
		<button type="button" class="btn btn-success radius" id="" name="" @click="allUser" ><i class="Hui-iconfont">&#xe665;</i> 搜用户</button>
	</div>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<tr class="text-c" v-for="(u,i) in user">
				...
				<td class="td-manage"><a style="text-decoration:none" @click="member_stop(this,u.id,u.locked)" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a> <a title="编辑" href="javascript:;" @click="member_edit('编辑','/edit?id='+u.id,'','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" @click="change_password('修改密码','/changepassword?id='+u.id,'10001','600','270')" href="javascript:;" title="修改密码"><i class="Hui-iconfont">&#xe63f;</i></a> <a title="删除" href="javascript:;" @click="member_del(this,u.id)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
		</tbody>
	</table>
	</div>
...
new Vue({
	  ...
	  methods: {
		...
		layer.confirm('确认要停用吗？',function(index){
			  ...
			  {
		           this_a=this;
  layer.confirm('确认要取消停用吗？',function(index){
						$.ajax({
							type: 'POST',
							url: '/use',
							data: {id:id},
		...
		change_password(title,url,id,w,h){
			layer_show(title,url,w,h);	
		},
		allUser(){
			 this_a=this;
		     ...
		     this_a.udate.end=document.getElementById('datemax').value;
	          ...
	          axios.post("/selectAllUser",this_a.udate)
	          .then(res => {
	            console.log(res);
	             this_a.user = res.data;
	          })
